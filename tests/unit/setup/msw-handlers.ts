...
export const handlers = [
    ...
    http.get('/api/v1/chats', () =>
        HttpResponse.json({
            chats: [
                { id: 'chat1', userId: 'user1', title: 'Chat 1', createdAt: new Date().toISOString() },
            ],
        }),
    ),
    ...
    http.get('/api/v1/chats/:chatId', ({ params }) =>
        HttpResponse.json({
            messages: [
                { id: 'msg1', content: 'Hello', role: 'USER', chat: null, chatId: params.chatId },
            ],
        }),
    ),
    ...
    http.post('/api/v1/onboarding/me/path/personalize', () =>
        new HttpResponse(
            sseStream(
                JSON.stringify({ type: 'stage', name: 'Analyzing', detail: 'Checking skills' }),
                JSON.stringify({
                    type: 'path',
                    path: {
                        id: 'newPath',
    ...
    http.put('/api/v1/onboarding/me/steps/:stepId/start', () =>
        new HttpResponse(null, { status: 200 }),
    ),

    http.post('/api/v1/onboarding/me/steps/:stepId/skips', () =>
        HttpResponse.json({ id: 'skip1', stepId: 'step1', status: 'PENDING', reason: 'Too hard', reviewComment: null, createdAt: new Date().toISOString() }),
    ),

    http.put('/api/v1/onboarding/me/tasks/:taskId', () =>
        new HttpResponse(null, { status: 200 }),
    ),
    ...
    http.post('/api/v1/users/:userId/project-roles', () =>
        new HttpResponse(null, { status: 200 }),
    ),
];
