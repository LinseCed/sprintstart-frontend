...
function mockAdminUsers() {
    server.use(
        ...
        http.get('/api/v1/github/pat', () =>
            HttpResponse.json(['default-token']),
        ),
    );
}
...
describe('AdminPage', () => {
    ...
    it('renders loaded users and filters them by search and status', async () => {
        ...
        await user.type(screen.getByRole('textbox', { name: 'Search users' }), 'jane');
    ...
    it('deletes selected users after confirmation', async () => {
        ...
        await user.click(screen.getByRole('checkbox', { name: 'Select Jane Smith' }));
