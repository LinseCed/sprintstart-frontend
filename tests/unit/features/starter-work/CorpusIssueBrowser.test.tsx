...
describe('CorpusIssueBrowser', () => {
    ...
    it('lists the project’s open issues with enough text to judge one', async () => {
        ...
        expect(
            screen.getByText(/users land on the wrong page after signing in/i)
        ).toBeInTheDocument();
        expect(screen.getByText('bug')).toBeInTheDocument();
    });

    /**
     * ⚠️ The copy must not read as picking which tasks hires are allowed to have — that is the gate
    ...
    /**
     * ⚠️ `hasAssignee` is three-valued and null means *we do not know* — SprintStart does not ingest
    ...
    it('shows an issue already in the pool as such, without offering it again', async () => {
        ...
        expect(
            screen.queryByTestId('promote-issue-github:acme/repo:ISSUE:1')
    ...
    /**
     * ⚠️ An empty corpus must not read as "no good first work here" — that is the exact silence the
