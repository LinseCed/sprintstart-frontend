...
import type { AdminProjectDetails, AdminUser, ProjectOverview } from '../../../../../src/features/admin/types';
...
describe('ProjectDetailsDrawer', () => {
    ...
    describe('unified dirty state', () => {
        ...
        it('counts detail and people changes together in one footer', async () => {
            ...
            await user.click(
                screen.getByRole('button', { name: /Remove Jane Doe from project/ }),
            );
        ...
        it('saves detail and people changes in one action', async () => {
            ...
            await waitFor(() =>
                expect(vi.mocked(projectService.updateProject)).toHaveBeenCalledWith(
                    'proj-1',
                    { name: 'Beta', description: 'Detailed project description' },
                ),
            );
            expect(
                vi.mocked(projectService.removeUserFromProject),
            ).toHaveBeenCalledWith('proj-1', 'u-1');
        });
    ...
    describe('people section', () => {
        ...
        it('shows a manager who has no membership row', async () => {
            ...
            await waitFor(() =>
                expect(screen.getByText('Tom Fischer')).toBeInTheDocument(),
            );
            expect(screen.getByText('Manager')).toBeInTheDocument();
        });
        ...
        it('stages an added user and saves it with the save bar', async () => {
            ...
            await user.type(
                screen.getByRole('textbox', { name: 'Search or add people' }),
                'tom',
            );
            ...
            await waitFor(() =>
                expect(vi.mocked(projectService.assignUsersToProject)).toHaveBeenCalledWith(
                    'proj-1',
                    { userIds: ['u-2'] },
                ),
            );
        ...
        it('promotes a member to manager and saves the assignment', async () => {
            ...
            await user.click(
                screen.getByRole('button', { name: /Make Jane Doe project manager/ }),
            );
            await user.click(screen.getByRole('button', { name: 'Save changes' }));
                ...
                users: [
                    {
                        ...
                        lastName: 'Klein',
                        roles: ['USER'],
                        projectRoles: [{ id: 'r-member', name: 'MEMBER' }],
                        enabled: true,
                    },
                ],
            });
