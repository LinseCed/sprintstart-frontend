...
const projects: ProjectOverview[] = [
    {
        ...
        users: [
            { id: 'u-1', username: 'a', email: 'a@x.com', projectRoles: [] },
        ],
    },
    {
        id: 'proj-2',
...
describe('ProjectsTab', () => {
    ...
    it('renders a card for each project', () => {
        ...
        expect(
            screen.getByRole('button', { name: 'Open details for Alpha' }),
        ).toBeInTheDocument();
        expect(
            screen.getByRole('button', { name: 'Open details for Beta' }),
        ).toBeInTheDocument();
    ...
    it('shows a fallback description when none is provided', () => {
        render(<ProjectsTab filteredProjects={projects} onOpenProjectDetails={vi.fn()} />);
        expect(
            screen.getByText('No project description available yet.'),
        ).toBeInTheDocument();
    });
