 ...
 *
 * ⚠️ **These live in their own file, and reset the module registry between tests, because the
...
describe('warming the buddy visit', () => {
    ...
    /**
     * ⚠️ The one request an unopened widget makes, and the exception is the point: writing the
    ...
    /**
     * ⚠️ `<React.StrictMode>` double-invokes effects in development, and two concurrent opens is
     * the exact shape of `backend#170`. The backend is idempotent per visit so a duplicate would
     * be harmless rather than wrong — this simply declines to send it.
     */
    ...
    /**
     * ⚠️ The race this closes is real rather than theoretical: the widget is mounted on `/buddy`
     ...
     *
     * ⚠️ **Asserted as ordering, not as a request count, and the distinction is the whole point.**
    ...
    it('makes a real open wait for a warm-up already in flight, rather than racing it', async () => {
        ...
        const { warmBuddyVisit, streamOpenBuddy } = await import(
            '../../../../src/services/buddyService'
        );
