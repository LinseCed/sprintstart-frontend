...
describe('IngestionMetrics', () => {
    ...
    it('renders subtitles for each metric card', () => {
        ...
        expect(
            screen.getByText('sources with at least one ingestion run'),
        ).toBeInTheDocument();
        expect(screen.getByText('from latest source statuses')).toBeInTheDocument();
        expect(
            screen.getByText('failed items from latest statuses'),
        ).toBeInTheDocument();
        expect(
            screen.getByText('not provided by current service'),
        ).toBeInTheDocument();
    });
