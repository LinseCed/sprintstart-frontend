...
function mockRequests(data: KnowledgeRequest[] | null, state: { loading?: boolean; error?: boolean } = {}) {
...
describe('MyEscalations', () => {
    ...
    it('shows the answer itself, not just that one arrived — the promise the button makes', () => {
        ...
        expect(
            screen.getByText('Ask in #platform and Dana will provision them.'),
        ).toBeInTheDocument();
    });
    ...
    it('does not treat an ANSWERED request with no answer body as answered', () => {
        ...
        mockRequests([request({ status: 'ANSWERED', answeredAt: '2026-07-27T10:00:00Z', answer: null })]);
        const { container } = render(<MyEscalations />);
        expect(container).toBeEmptyDOMElement();
    });
    ...
    it('separates the three outcomes when the hire has all of them', () => {
        mockRequests([
            request({ id: 'r1', status: 'ANSWERED', answeredAt: '2026-07-27T10:00:00Z', answer: answer() }),
