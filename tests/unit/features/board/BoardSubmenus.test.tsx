...
describe('taking a card into the conversation', () => {
                ...
                board={board([
                    {
                        ...
                        held: [
                            { competencyKey: 'kotlin', label: 'Kotlin', level: 3, targetLevel: 2 },
                        ],
                        inProgress: [],
                    },
                ])}
    ...
    it('offers nothing to ask about pull requests it cannot attribute', () => {
        render(
            <BoardGrid
                board={board([
                    { kind: 'OPEN_PULL_REQUESTS', pullRequests: [], attributionMissing: true },
                ])}
            />,
        );
...
describe('the cards the mentor places about progress', () => {
                ...
                board={board([
                    {
                        ...
                        inProgress: [
                            { competencyKey: 'testing', label: 'Testing', level: 1, targetLevel: 2 },
                        ],
                    },
                ])}
            />,
    ...
    it('says plainly when the buddy remembers nothing yet', () => {
        render(
            <BoardGrid
                board={board([{ kind: 'MEMORY_RECAP', memory: null, messagesRemembered: 0 }])}
            />,
        );

        expect(screen.getByText(/starts remembering after your first conversation/i)).toBeInTheDocument();
