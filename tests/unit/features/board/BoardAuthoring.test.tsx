...
const vocabulary: Board['vocabulary'] = {
...
describe('arranging the board', () => {
    it('moving a card sends the whole resulting order', () => {
        ...
        render(
            <BoardGrid board={board([note('first'), note('second')])} onReorder={onReorder} />,
        );
    ...
    it('cannot move the first card earlier or the last one later', () => {
        render(
            <BoardGrid board={board([note('first'), note('second')])} onReorder={vi.fn()} />,
        );

        expect(screen.getAllByRole('button', { name: /move the note card earlier/i })[0]).toBeDisabled();
