...
describe('TokensSection', () => {
    ...
    it('surfaces a server error in the error banner', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('settings-tokens-error')).toHaveTextContent(
                'Server 500',
            );
        });
    });
    ...
    it('adds a token with a valid PAT and refreshes the list', async () => {
        ...
        await user.type(
            screen.getByTestId('settings-add-token-value'),
            'ghp_aBcDef123',
        );
        await user.click(screen.getByTestId('settings-add-token-submit'));

        await waitFor(() =>
            expect(addGithubPat).toHaveBeenCalledWith('ci', 'ghp_aBcDef123'),
        );
        await waitFor(() =>
    ...
    it('rotates a token with a valid PAT', async () => {
        ...
        await user.type(
            screen.getByTestId('settings-rotate-token-default'),
            'github_pat_newvalue123',
        );
        await user.click(screen.getByTestId('settings-rotate-submit-default'));

        await waitFor(() =>
            expect(updateGithubPat).toHaveBeenCalledWith(
                'default',
                'github_pat_newvalue123',
            ),
        );
    ...
    it('marks the section aria-busy while refreshing', async () => {
        ...
        await waitFor(() =>
            expect(screen.getByLabelText('GitHub access tokens')).toHaveAttribute(
                'aria-busy',
                'true',
            ),
        );
