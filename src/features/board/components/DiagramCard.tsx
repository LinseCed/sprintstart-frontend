...
export function DiagramCard({
    ...
    return (
        ...
        >
            {current.summary && (
                <p className="mb-3 text-sm text-app-text-muted">{current.summary}</p>
            )}
            ...
            {hasPicture ? (
                ...
                <p className="text-sm text-app-text-muted">
                    {current.reason ??
                        'There is nothing in this project’s material to draw this from yet.'}
                </p>
            )}
        ...
        <div data-testid="diagram-list" className="space-y-3">
            <ul className="space-y-2">
                {content.nodes.map((node) => (
                    <li key={node.id} className="text-sm">
                        ...
                        {node.summary && (
                            <span className="text-app-text-muted"> — {node.summary}</span>
                        )}
            ...
            {content.edges.length > 0 && (
                <ul className="space-y-1 border-t border-app-border pt-2">
                    {content.edges
                        .filter(
                            (edge) => labelById.has(edge.fromId) && labelById.has(edge.toId),
                        )
                        .map((edge) => (
                            <li
                            ...
                            >
                                {labelById.get(edge.fromId)} {EDGE_KIND_WORDS[edge.kind]}{' '}
                                {labelById.get(edge.toId)}
