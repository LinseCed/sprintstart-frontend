...
export function MemoryRecapCard({
    ...
    return (
        <BoardCardFrame
            ...
            subtitle={
                memory
                    ? `From ${messagesRemembered} ${
                          messagesRemembered === 1 ? 'message' : 'messages'
                      } so far`
                    : undefined
            }
            onDismiss={onDismiss}
        ...
        >
            {memory ? (
                <>
                    <blockquote className="whitespace-pre-wrap border-l-2 border-app-border pl-3 text-sm italic text-app-text-muted">
                        {memory}
                    </blockquote>
                    <p className="mt-2 text-xs text-app-text-muted">
                        Your buddy&apos;s own notes, not a record — it picks up from these next time
                        you talk. If something here is wrong, tell it.
                    </p>
                </>
            ) : (
