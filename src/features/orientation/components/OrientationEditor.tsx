 ...
 */
export function OrientationEditor({ taskTitle, taskUrl, initial, onSave, onRevert, onClose }: OrientationEditorProps) {
        ...
        >
            <div className="flex max-h-full w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-app-border bg-app-bg">
                <header className="flex items-start justify-between gap-3 border-b border-app-border px-5 py-4">
                    <div>
                        <h2 className="text-base font-semibold text-app-text">Write this orientation yourself</h2>
                        <p className="mt-0.5 text-xs text-app-text-muted">
                            For “{taskTitle}”. Saving pins your words — the AI won&apos;t rewrite them.
                ...
                <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 overflow-hidden lg:grid-cols-2">
                    <form
                        ...
                        className="min-h-0 space-y-4 overflow-y-auto border-b border-app-border p-5 lg:border-b-0 lg:border-r"
                    >
                        <div>
                            <label htmlFor="orientation-summary" className="mb-1 block text-xs font-medium text-app-text">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <select
                                        ...
                                        className="rounded-lg border border-app-border bg-app-surface px-2 py-1.5 text-xs font-medium text-app-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
                                    >
                                        {STEP_ORDER.map((step) => (
                                ...
                                <input
                                    ...
                                    value={section.title}
                                    onChange={(event) => draft.updateSection(section.key, { title: event.target.value })}
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    value={section.body}
                                    onChange={(event) => draft.updateSection(section.key, { body: event.target.value })}
                                    ...
                                    {section.citations.map((citation) => (
                                        <div key={citation.key} className="flex items-center gap-1.5">
                                            <input
                                                ...
                                                className="w-1/3 rounded-lg border border-app-border bg-app-surface px-2 py-1.5 text-xs text-app-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
                                            />
                                            <input
                                                ...
                                                className="flex-1 rounded-lg border border-app-border bg-app-surface px-2 py-1.5 text-xs text-app-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
                                            />
                                            <button
                                                type="button"
                    ...
                    <div className="min-h-0 overflow-y-auto bg-app-surface-muted p-5">
                        <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-app-text-subtle">
                            What the hire will see
                        </p>
                        <OrientationPanel orientation={preview} isLoading={false} error={null} onRetry={() => {}} />
                    </div>
                </div>
...
function buildPreview(taskTitle: string, taskUrl: string | null, input: AuthorOrientationInput): MyOrientation {
