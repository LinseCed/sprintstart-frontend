 ...
 *
 * ⚠️ **It lives with the buddy, not with the board, and only belongs on surfaces a hire can reach.**
 * `openAiBuddy` is a no-op wherever the widget is not mounted, and the widget is mounted only for
 * `USER` profiles. Putting this on a PM surface — `/starter-work`, the module editor — would be a
 * control that silently does nothing, which is why it is not on any of them.
 */
