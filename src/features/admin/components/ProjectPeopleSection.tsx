...
import type { ProjectManager, ProjectRoleRef } from "../../../services/projectService";
...
type ProjectPeopleSectionProps = {
  ...
  /**
   * Role editing for people already on the project.
   *
   * Applies immediately rather than staging into the draft: a role hangs off an existing
   * assignment (the backend 404s otherwise), so it is only offered for members who are
   * already there and not staged for removal.
   */
  roleEditing?: {
    availableRoles: ProjectRole[];
    /** `${userId}:${roleId}` of the change in flight, so only its own control spins. */
...
type PersonRow = {
  ...
  /** What they hold *on this project*. Empty is a real state; absent means not loaded. */
          ...
          {visibleRows.map((row) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                {/* Only for people already on the project: a role hangs off an existing
                    assignment, so offering one to a staged add would 404 on save. */}
                {roleEditing && !row.isPendingAdd && !row.isPendingRemove && (
                  <ProjectRoleEditor
                    userId={row.id}
                    displayName={row.displayName}
                    heldRoles={row.projectRoles}
