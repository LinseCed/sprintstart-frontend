 ...
 */
export function CanonicalAnswerCard({ answer, onSave, readOnly = false }: CanonicalAnswerCardProps) {
            ...
            {editing ? (
                <div className="space-y-3">
                    <div>
                        ...
                        <AutoResizeTextarea id={questionId} value={question} onChange={setQuestion} minRows={1} maxRows={4} />
                    </div>
                    <div>
                        <label htmlFor={answerId} className="mb-1 block text-xs font-medium text-app-text-muted">
                            Answer
                        </label>
                        <AutoResizeTextarea id={answerId} value={body} onChange={setBody} minRows={3} maxRows={12} />
                    </div>
                    {status === 'error' && (
                ...
                <>
                    <div className="flex items-start justify-between gap-3">
                        ...
                        {!readOnly && (
                            <button
                                ...
                                className="flex shrink-0 items-center gap-1 rounded-lg text-xs text-app-text-muted transition-colors hover:text-app-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
                            >
                    ...
                    <p className="mt-2 whitespace-pre-wrap pl-6 text-sm text-app-text-muted">
                        {answer.answer}
                    </p>
