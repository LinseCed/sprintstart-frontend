...
function getStorageKey(userId: string, projectId: string) {
  return `kb_docs_${userId}_${projectId || "none"}`;
}
...
export function KnowledgeBasePage() {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      getStorageKey(profile.id, selectedProjectId),
      JSON.stringify(documents),
    );
  }, [documents, profile, selectedProjectId]);
  ...
  const loadDocuments = useCallback(
    async (isMounted = true) => {
      ...
      try {
        ...
        const cachedDocs = saved
          ? (JSON.parse(saved) as DocumentMetadata[])
          : [];
        ...
        if (isMounted) {
          setDocuments(() => {
            const pendingDocs = cachedDocs.filter(
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await knowledgeService.deleteDocument(id, profile.id, selectedProjectId);
  ...
  const canUseKnowledgeBase = Boolean(profile && selectedProjectId);
  ...
  return (
    <div className="min-h-screen bg-app-bg text-app-text">
      ...
      <main className="app-page-frame space-y-6 py-6 sm:space-y-10 lg:py-8">
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-3 font-semibold">
                  ...
                  Upload Complete: {batchResult.success} documents ingested,{" "}
        ...
        {!canUseKnowledgeBase && !isLoadingProjects ? (
          <div className="rounded-2xl border border-dashed border-app-border bg-app-surface-muted p-8 text-center text-sm text-app-text-muted">
            Select a project to view and upload knowledge base documents.
          </div>
        ) : null}
            ...
            <div className="space-y-6 rounded-2xl border border-app-border bg-app-surface p-8 shadow-xl">
              <div>
                ...
                <p className="mt-1 text-sm text-app-text-subtle">
                  Select .md files to add them to the selected project knowledge
                  base.
                </p>
              </div>
              ...
              <FileUploadZone
                ...
                isUploading={isUploading || !canUseKnowledgeBase}
              />
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between px-2">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium text-app-text-subtle transition-all hover:bg-app-surface-hover hover:text-app-text disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-focus"
              >
                <RefreshCw
